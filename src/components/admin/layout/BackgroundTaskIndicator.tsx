...
export default function BackgroundTaskIndicator() {
  const { isSyncingAI, aiSyncProgress, isSyncingVectors, vectorSyncProgress, isAutoCategorizing, autoCategorizeProgress } = useBackgroundTaskStore();

  const showAI = isSyncingAI && aiSyncProgress;
  const showVectors = isSyncingVectors && vectorSyncProgress;
  const showAutoCategorize = isAutoCategorizing && autoCategorizeProgress;

  if (!showAI && !showVectors && !showAutoCategorize) return null;
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-blue-500/20 flex items-center justify-center">
                    <List className="w-4 h-4 text-blue-400 animate-pulse" />
                  </div>
                  <div>
                    <p className="text-xs font-black text-white uppercase tracking-wider">Catégorisation IA</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">
                      {autoCategorizeProgress!.done} / {autoCategorizeProgress!.total} produits
                    </p>
                  </div>
                </div>
                <span className="text-xs font-black text-blue-400">{Math.round((autoCategorizeProgress!.done / autoCategorizeProgress!.total) * 100)}%</span>
              </div>
              <div className="h-1.5 bg-slate-900/80/5 rounded-full overflow-hidden">
                <motion.div
                  className="h-full bg-gradient-to-r from-blue-500 to-cyan-400"
                  initial={{ width: 0 }}
                  animate={{ width: `${(autoCategorizeProgress!.done / autoCategorizeProgress!.total) * 100}%` }}
                  transition={{ duration: 0.5 }}
                />
              </div>
              <p className="text-[9px] text-slate-400 mt-2 italic">Vous pouvez naviguer, l'opération continue...</p>
