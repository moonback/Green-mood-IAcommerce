...
import { Leaf, Mic, ChevronRight, Sparkles } from 'lucide-react';
            ...
            <div className="relative flex flex-col items-end gap-3 pr-4 sm:pr-8">
                ...
                <div className="group relative">
                    <motion.button
                        ...
                        aria-label="Ouvrir l'assistant vocal IA"
                        className={`relative flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 border rounded-[1.25rem] transition-all duration-500 overflow-hidden backdrop-blur-2xl before:pointer-events-none before:absolute before:inset-0 before:bg-gradient-to-b before:from-white/10 before:to-transparent ${isVoiceActive
                            ? isLightTheme
                    ...
                    >
                        ...
                        <div className="relative z-10 flex items-center gap-2 sm:gap-3">
                            <div className={`relative w-8 h-8 sm:w-9 sm:h-9 rounded-xl border flex items-center justify-center ${isLightTheme ? 'bg-slate-100 border-slate-200' : 'bg-zinc-950/70 border-white/10'}`}>
                                {isVoiceActive ? (
                                    <>
                                        <Mic className="w-4 h-4 sm:w-[18px] sm:h-[18px] text-emerald-300 animate-pulse drop-shadow-[0_0_8px_rgba(16,185,129,0.9)]" />
                                        <motion.div
                                            animate={{ scale: [1, 1.35, 1], opacity: [0.2, 0.5, 0.2] }}
                                            transition={{ duration: 1.7, repeat: Infinity }}
                                            className="absolute inset-0 bg-emerald-500/25 rounded-xl"
                                        />
                                    </>
                                ) : (
                                    <Leaf className={`w-4 h-4 sm:w-[18px] sm:h-[18px] transition-all duration-500 ${pulse ? 'text-emerald-300 scale-110 drop-shadow-[0_0_8px_rgba(16,185,129,0.9)]' : 'text-emerald-400'}`} />
                                )}
                                {unreadCount > 0 && (
                                    <motion.span
                                        initial={{ scale: 0 }}
                            ...
                            <div className="flex flex-col items-start">
                                ...
                                <span className={`inline-flex items-center gap-1 text-[8px] font-bold uppercase tracking-[0.14em] leading-none ${isLightTheme ? 'text-slate-500' : 'text-zinc-300'}`}>
                                    <Mic className="w-2.5 h-2.5 text-emerald-300" />
                                    {displaySubLabel}
                                </span>
                            </div>
                        </div>
